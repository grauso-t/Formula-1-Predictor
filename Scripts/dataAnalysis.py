...
list = [1, 11, 12, 13, 14, 15, 16, 17, 18, 19, 45, 50, 128, 53, 55, 58, 88, 111, 112, 113, 114, 115, 116, 117, 118, 119, 120, 122, 123, 124, 125, 127, 133, 134]

# Count the number of rows where 'statusId' is in the given list
count = len(ds[ds['statusId'].isin(list)])

# Print the total count
print("Number of completed laps: " + str(count))

# Group by 'driverId' and 'driver_name' and sum the number of laps
grouped_ds = ds.groupby(['driverId', 'driver_name']).size().reset_index(name='conteggio')

# Compute class weights
class_weights = compute_class_weight('balanced', classes=np.unique(grouped_ds['conteggio']), y=grouped_ds['conteggio'])
class_weight_dict = dict(zip(np.unique(grouped_ds['conteggio']), class_weights))

# Display the resulting DataFrame
print(grouped_ds)

# Save the grouped DataFrame to a CSV file
grouped_ds.to_csv('raggruppato.csv', index=False)

# Print the number of drivers who have completed at least 40 laps
print("Piloti che hanno effettuato almeno 40 giri: " + str(len(grouped_ds[(grouped_ds['conteggio'] >= 40)])))

# Print the number of drivers who have not completed at least 40 laps
print("Piloti che hanno effettuato meno di 40 giri: " + str(len(grouped_ds[(grouped_ds['conteggio'] < 40)])))

# Print the total number of drivers
print("Piloti totale: " + str(len(grouped_ds)))

# Apply oversampling specifically to the number of laps
X = grouped_ds.drop('conteggio', axis=1)
y = grouped_ds['conteggio']
